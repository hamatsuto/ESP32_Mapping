...
const int angle_devid = 360;

int LocalMap[angle_devid];
...
void setup() {
  ...
  if (readMPU6050(MPU6050_WHO_AM_I) != 0x68) {
    SerialBT.println("Failed to initialize IMU!");
    while (true) ;
  }
  SerialBT.println("MPU6050 IMU Setting Start");
  ...
  //キャリブレーション
  SerialBT.println("Calculate Calibration");
  ...
  SerialBT.print("offsetX : ");
  SerialBT.println(offsetX);
  SerialBT.print("offsetY : ");
  SerialBT.println(offsetY);
  SerialBT.print("offsetZ : ");
  SerialBT.println(offsetZ);

  //Ultrasonic
  //pinMode(PIN_TRIGGER, OUTPUT);
  //pinMode(PIN_ECHO, INPUT);
  //digitalWrite(PIN_TRIGGER, LOW);
  SerialBT.println("UltraSonic Distance Measurement");
  for (int i = 0; i < (sizeof(LocalMap)/sizeof(LocalMap[0])); i++){
    LocalMap[i] = -1;
  }
}
...
void loop() {
  ...
  LocalMap[(int)angleZ] = DistanceCm;
